...
public class TaskAssigningConfigUtil {
    ...
    public static DataIndexServiceClient createDataIndexServiceClient(ClientServices clientServices, TaskAssigningConfig config) {
        ...
        if (config.isKeycloakSet()) {
            ...
            if (config.getOidcClientId().isPresent()) {
                builder.clientId(config.getOidcClientId().get());
            }
            if (config.getOidcCredentialsSecret().isPresent()) {
                builder.clientSecret(config.getOidcCredentialsSecret().get());
            }
            if (config.getClientAuthUser().isPresent()) {
                builder.username(config.getClientAuthUser().get());
            }
            if (config.getClientAuthPassword().isPresent()) {
                builder.password(config.getClientAuthPassword().get());
            }
            ...
            if (config.getClientAuthUser().isPresent()) {
                builder.user(config.getClientAuthUser().get());
            }
            if (config.getClientAuthPassword().isPresent()) {
                builder.password(config.getClientAuthPassword().get());
            }
