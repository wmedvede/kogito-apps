...
public abstract class BaseKogito9007IT implements JobServiceHealthAware {
    ...
    private static void executeInstancesAndEnsureTermination(String processUrl, String processGetByIdQuery) {
        ...
        String processInstanceId1 = TestUtils.newProcessInstanceAndGetId(processUrl, EMPTY_DATA);
        String processInstanceId2 = TestUtils.newProcessInstanceAndGetId(processUrl, EMPTY_DATA);
        String processInstanceId3 = TestUtils.newProcessInstanceAndGetId(processUrl, EMPTY_DATA);

        // All the instances must finish in a period of time, otherwise the issue is still present.
        TestUtils.assertProcessInstanceHasFinished(processGetByIdQuery, processInstanceId1, AT_LEAST_SECONDS, AT_MOST_SECONDS);
        TestUtils.assertProcessInstanceHasFinished(processGetByIdQuery, processInstanceId2, AT_LEAST_SECONDS, AT_MOST_SECONDS);
        TestUtils.assertProcessInstanceHasFinished(processGetByIdQuery, processInstanceId3, AT_LEAST_SECONDS, AT_MOST_SECONDS);
    }
}
