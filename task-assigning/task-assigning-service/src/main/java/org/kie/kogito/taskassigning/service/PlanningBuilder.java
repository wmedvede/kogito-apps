...
public class PlanningBuilder {
    ...
    private List<PlanningItem> buildPlanningItems(User user) {
        List<TaskAssignment> taskAssignments = extractTaskAssignments(user, IS_NOT_DUMMY_TASK_ASSIGNMENT); //dummy tasks has nothing to with the jBPM runtime, don't process them
        Iterator<TaskAssignment> taskAssignmentsIt = taskAssignments.iterator();
        TaskAssignment taskAssignment;
        List<PlanningItem> result = new ArrayList<>();

        //TODO aplicar aquello de las distintas estrategias de publicacion, etc.
        //de momento si es el planning user se asigna todo
        int count = 0;
