...
The jobs service facilitates the scheduled execution of tasks in a cloud environment. These tasks are implemented by different independent services, and can be started by using any of the jobs service supported interaction modes.
...
* **Retry**: a retries configuration in case the job execution fails.
...
The following example shows the body of a potential patch operation that is executed on a previously created job.
...
To update a job by using the eventing api you must create a cloud event of type job.update that contains a subset of Job Resource properties to update [see](#patch).
